...
void draw() {
  ...
  float rightup = 0, yellow_rightdown = 0; //倒勾勾計算
  boolean green_part1 = false, green_ok = false;
  boolean yellow_part1 = false, yellow_ok = false; //倒勾勾判斷
  for(int i = 0; i < pt.size() - 1; i++) {
    ...
    if(p2.x - p.x > 0 && p2.y - p.y < 0) rightup += sqrt(dpx*dpx + dpy*dpy); //倒勾勾的左下右上
    if(yellow_part1 && p2.x - p.x > 0 && p2.y - p.y > 0) yellow_rightdown += sqrt(dpx*dpx + dpy*dpy); //倒勾勾的左上右下
    if(green_part1 == false && rightdown > 150) green_part1 = true;
    else if(green_part1 == true && green_rightup > 150) green_ok = true;
    if(yellow_part1 == false && rightup > 150) yellow_part1 = true; //左上右下先為否再判斷
    else if(yellow_part1 == true && yellow_rightdown > 150) yellow_ok = true; //左下右上達成再判斷左上右下
  }
  if(dpx > 100 && dpy < 40) stroke(255, 0, 0);
  else if(dpx < 40 && dpy > 100) stroke(0, 0, 255);
  else if(green_ok) stroke(0, 255, 0);
  else if(yellow_ok) stroke(255, 255, 0); //是勾勾就黃色
  else stroke(0);
  for(int i = 0; i < pt.size() - 1; i++) {
    PVector p = pt.get(i), p2 = pt.get(i+1);
